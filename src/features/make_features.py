...
def remove_punct(sentence):
    filtered_sentence = [''.join([c for c in w if not c in list(string.punctuation.replace("'", ""))]) for w in sentence]
    return filtered_sentence
...
def make_features(df, task, output_required=True):
    X = df["video_name"]
    delimiter = "'"
    X = [('  '.join([x + delimiter for x in i.split(delimiter) if x]))[:-1].split() for i in X]
    #X = [i[1:-1].replace('"', '').split(", ") for i in X]
